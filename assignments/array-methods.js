...
let ticketPriceTotal = runners.reduce((acc, currentVal) => currentVal.donation + acc, 0);
...
// Problem 1
let contactInfo = [];
runners.forEach((runner) => { contactInfo.push(`${runner.first_name} ${runner.last_name}: ${runner.email}`) });
console.log(contactInfo);
// Problem 2
let companies = runners.map((runner) => runner.company_name);
console.log(companies);
// Problem 3
let shirts = runners.map((runner) => runner.shirt_size);

let sortedShirts = shirts.sort();
